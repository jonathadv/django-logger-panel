...
__all__ = ["LEVELS", "ALL_LOG_LEVEL", "set_logger_level", "get_all_loggers", "get_all_log_level"]
...
def set_logger_level(log_name: str, log_level: str):
    ...
    try:
        ...
        raise LoggerPanelUnknownError(f"A unknown error happened while setting logger '{log_name}'") from err
...
def get_all_loggers() -> Dict[str, Logger]:
    ...
    loggers.update(
        {key: logger for key, logger in Logger.manager.loggerDict.items() if not isinstance(logger, PlaceHolder)})
    return loggers
